...
| Large error count in project but your file is clean | Use `likec4 validate --json --no-layout --file <path-from-cwd> <project-dir>` and check `filteredErrors`; text mode may still print upstream diagnostics |
...
1. **Run validation with `--file` and check `filteredErrors`:**
   ```bash
   likec4 validate --json --no-layout --file <path-from-cwd> <project-dir>
   ```
   `--file` paths are resolved from the command's current working directory. If `filteredErrors` = 0 but `totalErrors` > 0, the filtered files did not report errors; the error is elsewhere or the filter path did not match.
...
4. **Validate after each file edit.** Always run `likec4 validate --json --no-layout --file <path-from-cwd> <project-dir>` after changes; use `--file` to focus on your changes only and verify suspicious filters by checking `filteredErrors`.
